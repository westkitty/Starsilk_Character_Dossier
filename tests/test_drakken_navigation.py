...
def test_formerly_peripheral_drakken_are_typed_and_positioned_as_drakken():
    ...
    drakken_positions = [
        ...
        if r["id"] == "drakken-registry" or "drakken-page" in r["classes"].split()
    ]
...
def test_structural_count_locks_match_the_corrected_taxonomy():
    ...
    peripheral = sum(
        1 for r in sections if {"character-page", "peripheral-page"} <= set(r["classes"].split())
    )
