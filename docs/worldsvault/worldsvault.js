(() => {
  ...
  const render = () => {
    ...
    nodes.forEach(node => { const id = node.dataset.nodeId; node.classList.toggle('is-selected', id === selected); node.classList.toggle('is-connected', Boolean(selected && connected.has(id) && id !== selected)); node.classList.toggle('is-muted', Boolean(selected && id !== selected && !connected.has(id))); });
    ...
    if (showAllBtn) showAllBtn.setAttribute('aria-pressed', String(!selected));
  };
  ...
  showAllBtn.addEventListener('click', () => { selected = null; history.replaceState(null, '', location.pathname); render(); });
