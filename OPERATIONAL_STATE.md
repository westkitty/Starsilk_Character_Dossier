...
78. The Canon Loom is a deterministic claim-level evidence projection of the 120 authored `data-lore-record` entries in `src/content/sections/canon-ledger.body.html`. Claim IDs `C001`-`C120` retain authored ledger identity; `build/canon_loom.py` may expose source-backed claim records and explicit numeric atoms but must not infer stronger semantics. Source-claim contradiction, machine-lock conflict, and human canon approval remain separate states. `src/canon/canon-delta.schema.json` is an interchange contract only; exporting a Canon Delta never mutates canon. A no-conflict or no-source-contradiction result is never canon approval.
79. The Administration Educational Film Vault is curated diegetic production media, not canon/content authority. `src/films/films.json` may organize film identity, status, staging, narration, institutional framing, source pointers, and instructor material; cited canon remains governed by existing Compendium sources. Planned reels must remain visibly unfinished, archive dates remain unknown until authored, every reel remains Administration-issued, and the exact slogan `Your fate is in our hands.` is preserved. Generated `docs/films/` is disposable output owned by `build/film_publication.py`.
...
## Canon Loom claim evidence — VERIFIED / PUBLISHED

- Work branch: `feature/canon-loom`, based on `main` commit `a5bb1778f3edec0ab617d7faa72e1b105442e394`.
- `build/canon_loom.py` deterministically parses all 120 authored Canon Ledger records (`C001` through `C120`) with Python standard-library HTML parsing, preserving domain, tag, label, statement, source locator, xref identities, and explicit numeric atoms without creating a second canon database.
- Phase 7 context packets now carry `source_claims` as proof-carrying projections of matching authored Canon Ledger records. Empty claim arrays remain explicit evidence state; generated packets remain derivatives and do not become canon authority.
- Revision Chamber report schema v2 separates machine-lock conflicts from direct authored-source contradictions, exposes `Why Is This Canon?`, `What Breaks If This Changes?`, minimum proven sacrifice sets, evidence-ordered `Earliest Proven Divergence`, and exploratory Canon Delta export while retaining `HUMAN DECISION REQUIRED` whenever no explicit machine lock blocks a proposal.
- `src/canon/canon-delta.schema.json` defines the exploratory/provisional/accepted/locked interchange shape without adding any repository-write or automatic canon-promotion action.
- Agent evaluation now treats epistemic overreach from narrow evidence into stronger unsupported claims as a fail condition.
- Local deterministic build passed with `media/source/` temporarily excluded exactly as CI's source-only checkout does: 0 duplicate IDs, 0 broken anchors, 16 canon locks / 0 violations, 36 Drakken identity assertions / 0 failures, 0 manifest errors, and the 576-file public-boundary gate clean.
- Focused Canon Loom + Revision Chamber + agent evaluation proof passed `15/15`; supporting local suites passed `28/28` for agent/archive/provenance/canon/loom/chronology, `7/7` cold-start recovery, `6/6` derivation-map integrity, and `5/5` discovery. Plain system-Python `python3 build/generate.py --check` also passed after removing the accidental BeautifulSoup dependency from the Loom parser.
- First protected PR CI run `35159920821` proved Firefox and WebKit green and reached `256 passed, 1 skipped` in Chromium with no visual-regression artifacts; its sole failure was the pre-existing offline shell budget gate because duplicated inline Loom claim data raised precache size to `2,088,159` bytes (> `2,000,000`).
- Bounded repair removed the duplicated 120-claim corpus from Revision Chamber payload data. Browser analysis now reconstructs claim text/identity directly from the already-rendered authoritative `#canon-ledger` DOM and receives only a compact claim-ID numeric-fact map for explicit mismatch checks. No offline surface was removed. Generated root size fell from `931,964` to `841,076` bytes and total precache to `1,997,271` bytes; the exact failed offline-budget test passes locally, and focused Canon Loom/Revision/agent tests remain `15/15`.
- Final protected PR #70 CI run `35160560699` passed at the repaired head `bfd4689f502601541171726b85da9b682ff27038`: Chromium `257 passed, 1 skipped`; Firefox `13/13`; WebKit `13/13`. Chromium attempt 1 exposed a transient zero-height Reader Workbench touch-target signal that had passed the prior CI and then passed five consecutive local reproductions; rerunning the failed job on the identical commit (attempt 2, no code change) passed, so no unrelated Reader CSS/test mutation was made.
- PR #70 merged through protected `main` at `38c93724ef3e8499994f55f5f21225d94e6ebfdf` on 2026-09-16. Main CI run `35161132000` passed Chromium `257 passed, 1 skipped` plus Firefox `13/13` and WebKit `13/13`.
...
## Cross-Surface Record Explorer — IMPLEMENTED / LOCAL VERIFIED

- Branch: `feature/cross-surface-record-explorer`, created from current `origin/main` base `7f8d910` after proving `feature/administration-film-vault` is already an ancestor of `origin/main`; the old Film Vault branch was preserved untouched.
- `src/records/AUTHORITY.md` defines a derivative-only evidence join keyed exclusively by the 127 existing stable top-level record IDs. It creates no second canon, relationship, chronology, media, WorldsVault, Film Vault, or tour authority.
- `build/record_explorer.py` owns deterministic `docs/records/` generation. The full versioned JSON index classifies `authored-record`, `manifest-related-media`, `observed-xref`, `authored-chronology`, `machine-lock-reference`, `derived-context-packet`, `curated-tour-membership`, `authored-worldsvault-reference`, `authored-film-source-reference`, and `machine-alternative` evidence separately.
- Entity permalinks expose a static `Explore this record` context map plus a fully equivalent text/list representation. Observed relationship items remain `kind=mentions`; chronology attaches only to the explicitly authored `chronology` source record; machine locks attach only when `section_locks[].section` names the stable ID; WorldsVault and Administration Film links require explicit source stable IDs.
- `/records/` provides local search and facets for record type plus chronology, mention, media, lock, tour, WorldsVault, and film evidence presence. The offline shell precaches only the slim explorer UI and a compact 127-record search projection; the full 635 KB evidence index remains a published deterministic artifact but is deliberately not eagerly precached. The total precache remains below the existing 2 MB ceiling and no media binaries are added to cache.
- Machine orientation, sitemap, versioned schema publication, unified museum navigation, derivation topology, public-boundary scope, README orientation, entity integration, offline publication, and regression coverage are part of the same change set.
- At task start, strict validation exposed 47 local PNGs in `media/source/` that were absent from the committed manifest. During the validation workflow the local source directory later became absent. The authoritative 213-file manifest-backed canonical set was reconstructed from historical commit `97ae39c745933a024791ed75924f2a5d1d7844a5` using the repository's documented recovery method and then passed `tools/media_source_archive.py verify --json` at 213/213. The 47 pre-existing nonmanifest PNGs were outside repository/manifest authority and remain unrecovered after bounded exact-copy searches by filename and SHA-256 prefix across ordinary Mac user folders, Google Drive filename search, and recent ChatGPT Library image history. This unresolved local-work recovery issue is recorded separately from explorer correctness and is not disguised as a successful preservation result.
- Local verification is complete for the tracked feature: final focused explorer/machine/derivation/entity coverage passes 34/34; the offline museum suite passes 4/4 in serial Chromium; representative Chromium, Firefox, and WebKit matrices each pass 15/15. Firefox's first long-run unified-shell failure was isolated as transient: the exact journey passed 1/1 alone and the subsequent clean full Firefox rerun passed 15/15, so no unrelated navigation repair was made.
- No canon prose, stable ID, semantic relationship authority, authored chronology, canonical media binary, frontend framework, backend, account, analytics, remote search service, or Phase 13 is introduced. Protected PR CI, merge, Pages deployment, and live-edge proof remain separate unverified states until independently observed.
...
- Revision 35: closed the Witness Engine publication after PR #66 passed required CI run `35037008364` (pinned Chromium full suite plus Firefox/WebKit representative journeys) and merged to protected `main` at `be5a5bf24e219350f104b6a29c256a1ab6df7800`. Pages run `35037199618` requested and observed an exact legacy Pages build for that merge SHA, then passed live-edge publication checks. Independent cache-busted proof compared live root HTML and `service-worker.js` byte-for-byte with merged `docs/`: root SHA-256 `25a1d3e80d242ba2e2c344e3302ff55382b123c14a9c5784964cb8d407a5d77a`; service-worker SHA-256 `03edde6fb33a904ed2cfcad4a8fb105c1bebc9825cb2540ed57bce782f91975f`. The live root contains `id="witnessEngine"` and a decodable `starsilk-witness-data/1` evidence payload. No visual baseline, canon prose, relationship authority, media provenance, dependency, service, telemetry, or Phase 13 change was introduced.
- Revision 36: added the post-program Revision Chamber as a root-only browser-memory speculative canon-planning surface. It supports ephemeral named reality branches, Witness-draft forking, machine-lock collision detection, authored chronology touchpoints, observed-xref mention touchpoints, deterministic authority/derivative review surfaces, explicit 'What would I have to sacrifice?' packets, branch-vs-branch/current-canon comparison, and explicit JSON export/import without automatic persistence. A zero-conflict proposal always remains `HUMAN DECISION REQUIRED`; only explicit machine prohibitions yield `BLOCKED BY MACHINE LOCK`. No action can canonize, write the repository, infer semantic relationships, or treat the machine-lock subset as complete canon. Protected PR CI, merge, and live Pages proof remain pending; no Phase 13 was created.
- Revision 37: closed Revision Chamber publication evidence after protected PR #68. PR CI run `35141247581` passed the full pinned Chromium suite plus representative Firefox/WebKit journeys; the feature merged through protected `main` at `866e8bc5dc514c9b71a90b63d8122db4a4668471`. Pages run `35141546190` verified the exact legacy Pages source commit and live publication. Independent live-proof run `35141809802` then fetched the public root and service worker cache-busted and proved byte-for-byte parity with the exact merge checkout: root SHA-256 `3b5a3d0a7a0bd33bb966090c38e0d859cbf271afb77e8bab5dfd83dc363618fa`, service-worker SHA-256 `42f4710ffb035552deff6e297b32bcde63568029a5f626d744a4420c9bea9566`. The live root exposed `id="revisionChamber"`, the exact `Plan the rewrite. Do not touch reality.` boundary, the `What would I have to sacrifice?` control, and decoded `starsilk-revision-data/1` with 126 stable records, 16 machine locks, and 27 authored chronology events. The independent proof workflow existed only on the already-merged feature branch and was removed after proof; it never entered `main`. No canon prose, media provenance, semantic relationship authority, dependency, backend, telemetry, persistence, repository-write path, or Phase 13 was added by this closure. Pending work for Revision Chamber is none.
- Revision 38: added the post-program Canon Loom on `feature/canon-loom`: deterministic claim-level projection of all 120 authored Canon Ledger records; proof-carrying `source_claims` in AI context packets; Revision Chamber v2 source-contradiction, minimum-sacrifice, `Why Is This Canon?`, `What Breaks If This Changes?`, evidence-ordered divergence, and exploratory Canon Delta surfaces; plus an epistemic-overreach agent-evaluation penalty. Local deterministic build, public-boundary, focused browser tests, supporting recovery/derivation/discovery suites, and plain system-Python generator parity are verified. First protected CI run `35159920821` proved Firefox/WebKit green and Chromium `256 passed, 1 skipped` except the offline precache ceiling; the bounded repair eliminated duplicated inline claim data by deriving browser claim text/identity from the existing Canon Ledger DOM, restoring precache to `1,997,271` bytes without dropping offline surfaces. Protected CI rerun, merge, Pages publication, and live-edge proof remain pending. No canon prose, semantic relationship authority, media provenance, backend, telemetry, persistence, automatic canon promotion, or Phase 13 was introduced.
- Revision 39: closed Canon Loom publication evidence after PR #70. Final PR CI run `35160560699` passed at `bfd4689f502601541171726b85da9b682ff27038` with Chromium `257 passed, 1 skipped`, Firefox `13/13`, and WebKit `13/13`; the identical-commit Chromium rerun cleared one transient Reader Workbench geometry signal without code changes. PR #70 merged through protected `main` at `38c93724ef3e8499994f55f5f21225d94e6ebfdf`; main CI run `35161132000` passed the same browser matrix. Pages deployment `35161144356` succeeded, legacy Pages build `1220076563` reported exact commit `built`, and publish proof run `35161131998` attempt 2 verified the live edge after an initial Pages-build race. Independent cache-busted proof established byte parity for root SHA-256 `8ae0ca7bffe7f0f269525de815eaadb4af9563e3940738b7952d37fc4889c536` and service-worker SHA-256 `caae6c6a91b4d0a936815574ff591717e90f6d7218ab08a1a70912b699a4bc73`, plus exact Codec packet parity and live claim/evaluation markers. Canon Loom is now merged and published; no canon prose, semantic relationship authority, media provenance, dependency, backend, telemetry, persistence, automatic canon promotion, visual baseline, or Phase 13 was introduced.

## Administration Educational Film Vault — IMPLEMENTED / LOCAL VERIFIED

- Branch: `feature/administration-film-vault`.
- Added six Administration-issued reel identities under `src/films/films.json`; only `reel-001`, **When a Star Becomes a Lock**, carries a complete eight-scene production treatment and instructor questions. Reels 002-006 remain explicitly `planned`.
- Film source pointers reference existing stable Compendium records and do not create semantic relationship authority. Archive dates remain `unknown`; mid-century styling is presentation grammar rather than an in-universe date claim.
- `build/film_publication.py` owns deterministic `docs/films/` generation; machine orientation, sitemap, shared navigation, root exploration, public-boundary coverage, derivation topology, and regression tests are integrated.
- Local verification passed: Film Vault browser/source suite `6/6`; unified museum-shell suite `15/15`; root, Film Vault, machine publication, relationship, canon, discovery, tours, chronology, WorldsVault, entity, museum, offline, and agent deterministic checks all match generated output; derivation topology passes at 44 nodes / 96 edges; strict source-only validation reports 16 canon locks / 0 violations, 36 Drakken identity assertions / 0 failures, 0 duplicate IDs, 0 broken anchors, 0 missing assets, and 0 manifest errors; the full declared public-boundary surface passes across 581 text/machine files. The local `media/source/` originals were temporarily hidden only for the source-only strict-validation parity check and restored immediately afterward. Protected PR CI, merge, and live Pages proof are not yet claimed.
...
- Revision 44: introduced the ordinary post-program Cross-Surface Record Explorer as a deterministic stable-ID evidence join over existing canon-lock, chronology, observed-xref, media, tour, WorldsVault, Film Vault, discovery, and machine-publication evidence. Added `/records/`, entity-page context maps with accessible text equivalents, a versioned machine schema/index, offline-capable compact search/facets within the existing 2 MB precache ceiling, unified navigation, derivation-map coverage, and regression tests while preserving all evidence-class and unknown-state boundaries. Deterministic build/check now passes normally with the reconstructed and independently verified 213-file canonical source set, strict validation reports zero manifest/canon/Drakken/public-boundary defects, focused tracked-feature coverage passes 34/34, Chromium representative journeys pass 15/15, WebKit passes 15/15, and offline Chromium passes 4/4; the clean Firefox rerun passes 15/15. The 47 baseline nonmanifest local PNGs remain a separately disclosed recovery gap. The task branch is `feature/cross-surface-record-explorer` from authority base `7f8d910`; protected merge/live publication are not yet claimed.

## Dossier Deep Systems — IMPLEMENTED / LOCAL VERIFIED

- Branch: `feature/dossier-deep-systems`, layered on the locally verified Cross-Surface Record Explorer work. Authority remains the existing authored source/canon hierarchy; generated `docs/` surfaces remain disposable derivatives.
- Implemented all ten requested dossier upgrades as one evidence-preserving integration layer:
  1. **Canon Contradiction Observatory** — explicit source-backed contradiction/tension records with typed status; absence is explicitly non-negative evidence.
  2. **Authored semantic relationships** — a separate authored semantic-edge registry; existing `mentions / observed-xref` relationships remain unchanged and are never promoted automatically.
  3. **Universal command palette** — Ctrl/Cmd+K navigation across the Compendium, major museum systems, stable records, Witness Engine, and Revision Chamber.
  4. **Global temporal lens** — browser-session era selection shared across surfaces; only explicitly temporal evidence is filtered, while unknown temporal state remains visible.
  5. **Canon change history** — structured source-backed canon deltas with explicit unknown adoption dates where history is unavailable.
  6. **Claim/evidence tracing** — per-record “Why is this here?” evidence-class and source-pointer inspection.
  7. **Causality Loom** — separately authored causal edges with explicit causal type and claim evidence; chronological adjacency is never promoted to causation.
  8. **Visual generation packets** — deterministic per-record packets exposing stable source, existing media, applicable machine locks, authored archetype, and explicit non-inference rules.
  9. **Drakken comparative morphology atlas** — side-by-side strain comparison using authored archetype metadata while preserving unstructured anatomy/locomotion/scale/environmental axes as `unknown` rather than inferring them.
  10. **Portable Research Trails** — browser-local question + stable-ID trails with JSON/Markdown export and an explicit non-canon authority boundary.
- New source authority for these cross-surface assertions is `src/records/deep-systems.json`, validated by `src/schema/deep-systems.schema.json`. Deterministic derivatives are published through `docs/records/deep-systems.json` and its schema, while the machine publication also exports the versioned schema.
- Global palette assets are generated root assets (`global-tools.js/css`). Deep record-analysis assets (`records-deep.js/css`) are deliberately not added to the offline precache; the prior compact offline record search remains fully cached. This preserves the hard 2 MB offline ceiling without deleting an existing offline surface.
- Local verification:
  - full deterministic build + strict validator: PASS;
  - duplicate IDs 0; broken anchors 0; external runtime dependencies 0; canon invariant violations 0; Drakken identity defects 0; manifest defects 0;
  - public derivative boundary: 590 files PASS;
  - focused/source/machine/offline/unified-shell suite: 48 PASS;
  - representative Chromium cross-browser journey suite: 15 PASS;
  - dedicated deep-systems browser journey verifies Ctrl/Cmd+K, temporal persistence, semantic/contradiction/causality panes, Drakken comparison, research trail, and evidence trace;
  - offline precache: 1,996,773 bytes, below the 2,000,000-byte ceiling;
  - derivation topology: 46 nodes / 105 edges PASS;
  - `git diff --check`: PASS.
