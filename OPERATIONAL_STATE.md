...
## Truth Lab epistemic analysis suite — IMPLEMENTED / LOCAL VERIFIED

- Implemented as a root-only Reader Workbench extension over the existing Canon Loom, chronology, stable-record, and revision evidence seams. It is post-program capability work, not Phase 13.
- The **Parallax Engine** filters exact authored Canon Loom claims and permits browser-memory-only perspective assignments. Those assignments are reader analysis, not authored speaker metadata, semantic relationship authority, or canon.
- **Truth Horizon** compares only authored chronology phase structure and directly authored temporal markers. It is an archive-evidence horizon and must never be described as proof of what any character personally knew.
- **The Silence Index** exposes explicit chronology unknowns plus established inference prohibitions. Ordinary absence is not automatically promoted into mystery, contradiction, or a negative canon fact.
- **Canon Quarry** breaks user-supplied text into exploratory candidate propositions and returns lexical evidence neighbors from the authored Canon Ledger. Lexical overlap is not semantic equivalence, contradiction proof, or canon promotion.
- **Canon Change Receipt** compares before/after text and emits a bounded review receipt. Its review surfaces are a mechanically provable subset from matched claim/entity evidence, never a complete dependency graph and never proof of causation.
- All Truth Lab state is memory-only unless the user explicitly downloads an export. The implementation contains no localStorage, sessionStorage, fetch, XMLHttpRequest, backend, telemetry, repository-write path, or automatic canon-promotion action.
- Truth Lab behavior/style are generated as separate same-origin root assets (`truth-lab.js` / `truth-lab.css`), mirroring the Wound Graph seam so the existing narrow offline precache budget is not expanded. The cached root remains usable if optional root analysis assets are unavailable offline.
- Local source/output proof: JavaScript syntax, Python generator compilation, `git diff --check`, root generation, and root generator `--check` pass. Focused Truth Lab Chromium regression coverage passes 8/8 when pointed at the repository's already-installed Playwright Chromium bundle; Canon Loom focused tests pass 4/4. The full deterministic build/check also passes: every generated subsystem matches, strict validation reports 0 duplicate IDs, 0 broken anchors, 0 missing local assets, 0 canon-lock violations, 0 JavaScript syntax errors, 0 manifest errors, and the public-boundary gate passes across 581 text/machine files.
- No canon prose, machine canon lock, semantic relationship authority, chronology source data, media provenance, dependency, service, visual baseline, or existing published identity is changed by this feature.
...
- Truth Lab is implemented and locally build-verified in the isolated worktree. No branch, commit, push, pull request, merge, Pages deployment, or live-edge publication is claimed for Truth Lab yet.
- Codec Wound Graph remains merged, published, and independently live-edge verified. Museum + AI remains complete at Phase 12 of 12; both Wound Graph and Truth Lab are post-program capability work, not Phase 13.
...
- Revision 44: introduced the post-program Truth Lab as one root-only epistemic analysis suite containing Parallax Engine, Truth Horizon, Silence Index, Canon Quarry, and Canon Change Receipt. All five surfaces reuse authored Canon Loom/chronology/stable-record evidence, preserve UNKNOWN and observed-xref boundaries, keep local classifications in browser memory only, and have no backend, telemetry, automatic persistence, repository-write path, or canon-promotion path. Focused Truth Lab Chromium journeys pass 8/8 against the existing local Playwright browser bundle; Canon Loom tests pass 4/4; deterministic root generation/check, source syntax/diff checks, every generated subsystem check, strict validation, and the 581-file public-boundary gate pass locally. No Phase 13 was created.
